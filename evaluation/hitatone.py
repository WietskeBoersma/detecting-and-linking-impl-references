...
basename = os.path.basename(predicted_source)
basename_no_ext = os.path.splitext(basename)[0]
minute_id_base = basename_no_ext.replace('_recon_2', '')

true_data = []
with open(true_source, 'r', encoding='utf-8') as f:
    reader = csv.DictReader(f)
    for row in reader:
        if row.get("minute_id", "").startswith(minute_id_base):
            true_data.append({
...
def get_hit_and_predicted_doc(search_term):
    ...
    for gold in true_data:
        ...
        if gold["external_identifier"] == pred_id and score > best_score:
            best_score = score
            best_match = gold
...
# add new columns
if "hit-at-1-doc" not in df_pred.columns:
    df_pred["hit-at-1-doc"] = pd.NA
if "predicted_first_document" not in df_pred.columns:
    df_pred["predicted_first_document"] = ""

df_pred = df_pred[df_pred["minute_id"] == minute_id_base].copy()
for idx, row in df_pred.iterrows():
    search_term = row["zoekterm"]
    hit, pred_doc = get_hit_and_predicted_doc(search_term)
    df_pred.at[idx, "hit-at-1-doc"] = hit
    df_pred.at[idx, "predicted_first_document"] = pred_doc
...
df_pred.to_csv("df_1.csv", index=False)
